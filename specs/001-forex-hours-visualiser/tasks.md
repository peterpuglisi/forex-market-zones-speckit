...
- Source under `frontend/src/` â€” components, lib, hooks, app, __tests__
...
- [X] T004 [P] Add test scripts to `package.json`: `test`, `test:watch`, `test:coverage`; configure Vitest coverage thresholds (â‰¥80% global, 100% for `src/lib/forex-sessions/**`) in `vitest.config.ts`
...
- [X] T007 [P] Create `TRADING_SESSIONS` constant array with all four sessions and correct UTC hours (Sydney 22â€“07, Tokyo 00â€“09, London 08â€“17, New York 13â€“22) with Tailwind color assignments in `src/lib/forex-sessions/sessions.ts`
- [X] T008 Create `OVERLAP_WINDOWS` constant array with all three windows (Sydney/Tokyo 00â€“07, Tokyo/London 08â€“09, London/New York 13â€“17) in `src/lib/forex-sessions/sessions.ts` (depends on T007 â€” same file)

**Checkpoint**: Types and constants ready â€” user story implementation can begin

---

## Phase 3: User Story 1 â€” View Current Session Status (Priority: P1) ðŸŽ¯ MVP
...
### Tests for User Story 1 (MANDATORY â€” write and verify failing BEFORE implementation)
...
- [X] T014 [US1] Implement `SessionCard` component: session name, open/closed `Badge`, `minutesUntilChange` label, session `color` token as status indicator â€” no inline styles in `src/components/forex-hours/SessionCard.tsx` (depends on T013, T011 test must be written and failing first)
...
**Checkpoint**: `npm run dev` â†’ session status cards visible; at UTC 10:00 only London shows
open. User Story 1 fully functional and independently testable.

---

## Phase 4: User Story 2 â€” Identify Session Overlap Windows (Priority: P2)

**Goal**: Show a 24-hour timeline with visually distinct overlap zones

**Independent Test**: At 14:00 UTC the 13:00â€“17:00 London/New York overlap zone must be
visually highlighted on the timeline. At 05:00 UTC the Sydney/Tokyo zone (00:00â€“07:00) must
be highlighted. Confirms US2 works alongside US1.

### Tests for User Story 2 (MANDATORY â€” write and verify failing BEFORE implementation)
...
- [X] T018 [P] [US2] Write component tests for `SessionTimeline`: renders 24 grid columns, session bars span correct column ranges, Sydney bar renders as two segments (cols 23â€“24 + cols 1â€“7), an overlap zone is visually distinct from a solo-session zone, the current-time cursor is present in `src/__tests__/components/SessionTimeline.test.tsx`
...
## Phase 5: User Story 3 â€” Track Live Time (Priority: P3)
...
### Tests for User Story 3 (MANDATORY â€” write and verify failing BEFORE implementation)
...
- [X] T028 [P] Verify responsive layout on mobile viewport (â‰¤ 768px) using browser devtools; fix any overflow or unreadable text in `src/components/forex-hours/SessionTimeline.tsx` and `src/components/forex-hours/SessionCard.tsx`
...
- [X] T030 [P] Fix spec.md errata: update edge cases section to correct session times (Sydney 22:00â€“07:00, Tokyo 00:00â€“09:00) in `specs/001-forex-hours-visualiser/spec.md`
- [X] T031 Run full test suite and verify coverage thresholds are met (â‰¥80% global, 100% `src/lib/forex-sessions/`) â€” `npm run test:coverage`; fix any gaps
...
- [X] T034 [P] Install `@lhci/cli` and add a `lighthouse` script to `package.json`; run Lighthouse CI against `npm run build` output; assert Performance score â‰¥ 90 and initial load â‰¤ 2 000 ms; record result as the baseline in `lhci.config.js` (constitution Principle IV: performance MUST be verified via CI benchmark)
...
- **Setup (Phase 1)**: No dependencies â€” start immediately; T002â€“T005 parallel with T001
...
- **User Story 2 (P2)**: Can start after Foundational â€” no dependencies on US1 or US3
- **User Story 3 (P3)**: Can start after Foundational â€” T027 (wiring) requires US1 + US2 complete
...
# After all three tests fail â€” begin implementation:
Task: "Implement isSessionOpen in src/lib/forex-sessions/session-status.ts"                # T012
# then T013 â†’ T014 â†’ T015 â†’ T016 (sequential, each depends on prior)
```
...
3. Write tests T009â€“T011, verify all fail
4. Complete Phase 3: User Story 1 (T012â€“T016)
5. **STOP and VALIDATE**: At UTC 10:00 â†’ only London open; at 13:00 â†’ London + New York open
6. Demo/deploy MVP

### Incremental Delivery

1. Setup + Foundational â†’ run `npm test` (tests should fail â€” correct)
2. User Story 1 â†’ make tests pass â†’ `npm run dev` confirms session cards work (**MVP**)
3. User Story 2 â†’ timeline + overlap zones visible
4. User Story 3 â†’ live clock wired in; all auto-update
5. Polish â†’ coverage gate, build gate, Lighthouse CI baseline
...
- Both complete â†’ Developer A adds User Story 3 (wires live clock into integrated component)
...
- T008 is intentionally NOT marked [P] â€” it writes to the same file as T007 (`sessions.ts`)
- `useClock` accepts `intervalMs` parameter specifically to enable fast fake-timer tests in T023
- Sydney midnight-crossing: two grid segments â€” do not skip this; they must render as one visual bar
